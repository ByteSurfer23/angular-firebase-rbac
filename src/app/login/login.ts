...
@Component({
  ...
  <div class="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-6">
    <form (ngSubmit)="login()" class="bg-white p-6 rounded-xl shadow-md w-full max-w-md space-y-4">
  ...
  async login() {
    try {
      ...
      for (const orgDoc of orgsSnap.docs) {
        const userRef = doc(this.firestore, `organizations/${orgDoc.id}/users/${uid}`);
