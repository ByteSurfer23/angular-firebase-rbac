    ...
    <div class="min-h-screen bg-gray-900 text-gray-100 font-inter p-4 sm:p-6 rounded-xl overflow-hidden">
      ...
      <main class="flex flex-col lg:flex-row gap-6">
        ...
        <section class="lg:w-1/2 bg-gray-800 p-6 rounded-xl shadow-lg flex flex-col">
          ...
          <form (ngSubmit)="createProject()" class="space-y-6 mb-8 pb-8 border-b border-gray-700">
            <h3 class="text-2xl font-bold text-center text-blue-300">Create New Project</h3>
            ...
            <div class="border border-gray-700 p-4 rounded-md">
              ...
              <ul *ngIf="projectAssets.length > 0" class="list-disc pl-5 space-y-2 overflow-y-auto max-h-48 custom-scrollbar">
          ...
          <section class="mt-8">
            <h3 class="text-2xl font-bold text-center text-blue-300 mb-4">Search Existing Projects</h3>
            <div class="mb-4">
              <input
                type="text"
                ...
                <tbody class="text-gray-300 text-sm font-light">
                  <tr *ngFor="let project of filteredProjects" class="border-b border-gray-700 hover:bg-gray-700">
                    ...
                    <td class="py-3 px-6 text-center">
                      <button (click)="selectProject(project)" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-200 ease-in-out transform hover:scale-105">
                        Select
                      </button>
                    </td>
                  </tr>
        ...
        <section *ngIf="selectedProject" class="lg:w-1/2 bg-gray-800 p-6 rounded-xl shadow-lg flex flex-col">
          <h3 class="text-2xl font-bold text-center text-blue-300 mb-4">
            Tasks for: <span class="text-green-300">{{ selectedProject.name }}</span>
            <button (click)="selectedProject = null; resetTaskForm()" class="ml-4 text-gray-400 hover:text-gray-200 text-sm">
              (Clear Selection)
            </button>
          </h3>

          <!-- Task Creation/Edit Form -->
          <form (ngSubmit)="editingTask ? updateTask() : addTask()" class="space-y-4 mb-8 pb-8 border-b border-gray-700">
            <h4 class="text-xl font-semibold text-blue-300 mb-3 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2 text-green-300">
            ...
            <div>
              <label for="taskDescription" class="block text-gray-300 text-sm font-bold mb-2">Task Description:</label>
              <input type="text" id="taskDescription" [(ngModel)]="currentTaskDescription" name="currentTaskDescription" placeholder="Task Description" required
                     class="w-full p-3 border border-gray-700 rounded-md bg-gray-900 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" id="taskAssignedUserEmailInput" [(ngModel)]="currentTaskAssignedUserEmailInput" name="currentTaskAssignedUserEmailInput" placeholder="Enter emails to assign"
                     class="w-full p-3 border border-gray-700 rounded-md bg-gray-900 text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <!-- Optional: Add a dropdown/autocomplete for user search suggestions here -->
              <div *ngIf="currentTaskAssignedUserEmailInput && filteredDomainUsers.length > 0" class="bg-gray-700 rounded-md mt-1 max-h-40 overflow-y-auto custom-scrollbar">
                <div *ngFor="let user of filteredDomainUsers"
            ...
            <div class="flex gap-4">
              ...
              <button *ngIf="editingTask" type="button" (click)="resetTaskForm()"
          ...
          <section class="mt-8 flex-grow flex flex-col">
            ...
            <div class="overflow-y-auto flex-grow custom-scrollbar">
              <table class="min-w-full bg-gray-900 rounded-md overflow-hidden">
                ...
                <tbody class="text-gray-300 text-sm font-light">
                  <tr *ngFor="let task of filteredTasks; let i = index" class="border-b border-gray-700 hover:bg-gray-700">
                    <td class="py-3 px-6 text-left">{{ task.description }}</td>
                    <td class="py-3 px-6 text-center">
                      <span [ngClass]="{'text-yellow-400': task.status === 'in progress', 'text-green-400': task.status === 'completed', 'text-gray-400': task.status === 'not yet started'}">
                        {{ task.status | titlecase }}
                      </span>
                    </td>
                    <td class="py-3 px-6 text-center">{{ task.dueDate.toDate() | date:'shortDate' }}</td>
                    <td class="py-3 px-6 text-center">{{ getAssignedEmails(task.assignedTo) }}</td>
                    <td class="py-3 px-6 text-center">
                      <div class="flex item-center justify-center space-x-2">
                        <button (click)="editTask(task)" class="w-8 h-8 rounded-full bg-blue-600 hover:bg-blue-700 text-white flex items-center justify-center transition">
                          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4">
                            <path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/>
                          </svg>
                        </button>
                        <button (click)="confirmDeleteTask(task)" class="w-8 h-8 rounded-full bg-red-600 hover:bg-red-700 text-white flex items-center justify-center transition">
                          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4">
                  ...
                  <tr *ngIf="filteredTasks.length === 0">
                    <td colspan="5" class="py-4 text-center text-gray-500">No tasks found for this project.</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
export class CreateProjectComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.orgId || !this.domainUid) {
        this.showTemporaryMessage('Error: Organization ID or Domain ID missing. Cannot manage projects.', true);
    }
  ...
  async fetchDomainUsers(): Promise<void> {
    ...
    try {
      ...
      const admins = adminsSnap.docs.map(doc => ({ uid: doc.id, email: doc.data()['email'] }));

      this.allDomainUsers = [...users, ...admins];
    } catch (error) {
      console.error("Error fetching domain users/admins:", error);
      this.showTemporaryMessage("Failed to load domain users for task assignment.", true);
    }
  }

  get filteredDomainUsers(): AssignedUser[] {
    const inputEmails = this.currentTaskAssignedUserEmailInput.split(',').map(e => e.trim().toLowerCase());
  ...
  addAssigneeToInput(email: string): void {
    let currentEmails = this.currentTaskAssignedUserEmailInput.split(',').map(e => e.trim()).filter(e => e);
    ...
    this.currentTaskAssignedUserEmailInput = currentEmails.join(', ') + (currentEmails.length > 0 ? ', ' : '');
  }
  ...
  async updateTask() {
    if (!this.selectedProject || !this.editingTask) {
      this.showTemporaryMessage('No project selected or task being edited.', true);
      return;
    }
    if (!this.currentTaskDescription || !this.currentTaskDueDate || !this.currentTaskStatus) {
      this.showTemporaryMessage('Task description, due date, and status are required.', true);
  ...
  async deleteTaskConfirmed(taskToDelete: ProjectTask): Promise<void> {
    ...
    this.projectTasks = this.projectTasks.filter(task => task !== taskToDelete);
    ...
    if (this.projectTasks.length < initialTaskCount) {
      ...
      this.showTemporaryMessage(`Task "${taskToDelete.description}" deleted successfully!`);
    } else {
  ...
  editTask(task: ProjectTask): void {
    ...
    this.currentTaskReminderDate = task.reminderDate ? task.reminderDate.toDate().toISOString().split('T')[0] : '';
    this.currentTaskStatus = task.status;
    this.currentTaskAssignedUserEmailInput = this.getAssignedEmails(task.assignedTo);
  }
  ...
  get filteredTasks(): ProjectTask[] {
    ...
    return this.projectTasks.filter(task =>
      ...
      task.assignedTo.some(user => user.email.toLowerCase().includes(lowerCaseSearchTerm)) ||
   ...
   */
  private async resolveAssigneeEmails(emailString: string): Promise<AssignedUser[]> {
    const assigneeEmails = emailString.split(',').map(e => e.trim()).filter(e => e);
    const resolvedUsers: AssignedUser[] = [];

    for (const email of assigneeEmails) {
      let foundUid = '';
      const foundUser = this.allDomainUsers.find(u => u.email.toLowerCase() === email.toLowerCase());
      if (foundUser) {
        foundUid = foundUser.uid;
      } else {
        console.warn(`Assignee ${email} not found in domain users/admins. Adding with empty UID.`);
      }
  ...
  private async updateProjectTasksInFirestore(): Promise<void> {
    if (!this.selectedProject || !this.orgId || !this.domainUid) {
      this.showTemporaryMessage('Cannot save tasks: No project selected or domain info missing.', true);
      return;
    }
    try {
      const projectDocRef = doc(this.firestore, `organizations/${this.orgId}/domain/${this.domainUid}/projects/${this.selectedProject.uid}`);
      ...
      const projectIndex = this.projectsList.findIndex(p => p.uid === this.selectedProject?.uid);
      if (projectIndex > -1) {
